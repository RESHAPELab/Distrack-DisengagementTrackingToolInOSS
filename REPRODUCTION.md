...
This document is for developers who might work on or contribute to this project in the future. First I will explain how to run the program end to end then hand you context, background, and ideas about the project. Things like where we started, what we explored, and where I think it's going, along with the tips and hard-won lessons I wish someone had handed me. If you have questions I'd rather talk than have you guess, so a quick Zoom call will probably save hours. If you're just replicating, stop after Step 4. If you're contributing, read the Limitations section first to understand the trade-offs.

One note on how this is laid out. We will start with running it and then everything past that will be the deeper story, the why and the how and if you are attempting to contribute more.

# Getting it running 

We have 4 steps that you will need to run these are all contained in 3 different programs. This programs idea is to be able to predict developers disengagement and give insights on how to help effects of the departure. This is why we need 4 steps. Step one is getting the past data on developers to predict their future. Then we need to take our raw data and give some structre to it so our computers can learn. Then we set the computers to model and train on the data we collced. Lastly we now have all the data on activty, the fucture forcast of actity and insights from the strcuted data we made and need to serve it to the user.
...
Last step to running collection!
...
**Resume:** just re-run the same command. Each repo stores durable progress in  `data_cursor.json` / `scheduler_state.json`, so a re-run skips finished streams and continues mid-stream from the last page. Safe to Ctrl-C and restart anytime.**A repo errors out:** the run logs `[ERROR] ... Skipping <repo>` and continues to the next repo. Check the tail of the log for any skipped repos and re-run to retry them. **Force a clean re-pull of a stream**
...
When this stage finishes, each project folder contains: `commit_list.csv`, `per_file_commits.csv`, `issues.csv`, `issue_activity.csv`, `prs_repo.csv`, `prs_comments.csv`, `repo_tree.csv` (plus the cursor/scheduler JSONs).
...
2. Fill the queue. Simplest path is the **Add Training Repos** and **Add Test Repos** buttons (or **Add all to Queue**), which pull from `repo_split.csv`
3. Press **Predictors** (recommended). This walks every queued repo through the whole pipeline truck factor → daily timeline → state labeling → social-technical, project-health, and knowledge-distribution features → merged enriched timeline.
4. The **Responce** button is the lighter version: it produces the `state` column with only daily activity counts as predictors. Use **Predictors** for the full feature set the model actually trains on.


Tips: Caching is on by default a repo whose enriched timeline already exists is reloaded, not recomputed. To force a fresh run, switch on **Enable per-step overwrite controls** and toggle the specific step. The Social-Technical Network step is the slow one. Process BOTH train and test repos  because Step 3 reads every processed repo from `repo_split.csv`, so any repo you skip now will be missing at training time.
...
3. Set the **Prediction framing** (Binary state is the recommended default): 4-class means the model has 4 final nodes. each node represents the predictor of each class in 14 day horison(Active, Non Coding, Inactive and gone). If you want to represent the models output use a different graph that shows the probability of each class. The binary state compresses the 4 states into a single "active or inactive" insted of 4 different classes. 
...
Hopefully we have gotten to this step Ok and just need to run the DISTRAC app, it is a standalone tool with its own FastAPI backend. We do not need to provide anything to this app by hand as everthing should be provided from the pipeline. It reads the predictions written in Step 3 and serves an interactive dashboard the app explains itself once it's open. Hopefully you have seen a video of its use or read the paper and can see more details there.
...
Okay, the machinery. I'll keep this part lighter, both because you'll watch it happen when you run the program and because at the end of the day all of it exists just to solve one annoying problem: getting a lot of activity out of GitHub without GitHub cutting you off. The main program you run is "CommitExtratorV3", and that's the main file that kicks everything off. The reason for the multiple API tokens is that GitHub limits how many requests a single token can make in an hour. One token trying to collect a big project like React would take forever. So the program runs several workers in parallel, each one using a different token, which multiplies how fast you can pull data. To keep those workers from stepping on each other and collecting the same thing twice, there's a scheduler, "Data Collection\SimpleScheduler.py" whose entire job is to hand out the work and let each worker "claim" a page of results, so two workers never grab the same page. While the collection is running, you'll get a progress bar not for the scheduler, so you can see how far along it is.
...
To implement these three dimensions, I built on existing research. I used Pedro's work for the sociotechnical network. I used Truck Factor code for knowledge distribution and technical risk. Lastly, I tried using pre-tests and post-tests but landed on simple CHOSS metrics for historical break patterns.
...
If you understand the pipeline now and you're thinking about what to improve. The goal of the project is to make a dashboard or use data to help combat developer inactivity in OSS projects. We have two paths we can follow.
