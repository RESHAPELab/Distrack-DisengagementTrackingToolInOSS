...
def build_risk_table(repo_full_name: str, selected_date: datetime.date) -> pd.DataFrame | None:
    ...
    # Determine risk column: survival model uses risk_30d; LSTM uses prob_1
    has_survival_preds = "risk_30d" in test_df.columns or "S_7" in test_df.columns
    if has_survival_preds:
        risk_col = "risk_30d" if "risk_30d" in test_df.columns else None
        if risk_col is None:
            # derive from S_30
            test_df["risk_30d"] = (1 - test_df["S_30"].fillna(1)).clip(0, 1)
            risk_col = "risk_30d"
    else:
        risk_col = "prob_1" if "prob_1" in test_df.columns else None

    if risk_col is None:
        st.warning(
            "No risk predictions found in the data.  \n"
            "**Fix:** Re-run the prediction pipeline to generate survival or LSTM predictions."
        )
        return None

    cutoff = pd.Timestamp(selected_date)
    current_probs = (
        test_df.dropna(subset=[risk_col])
        .loc[lambda df: df["date"] <= cutoff]
        .sort_values("date")
        .groupby("dev")[risk_col]
        .last()
    )
...
def _make_state_graph(df: pd.DataFrame, repo_full_name: str = ""):
    ...
    has_survival  = "S_7" in df.columns
    ...
    BG            = "#1c1c2e"
    n_panels      = 4 if (has_probs or has_survival) else 3
    height_ratios = [0.4, 2, 2, 2] if (has_probs or has_survival) else [0.4, 2, 2]
    ...
    # Panel 3 — survival predictions (preferred) or LSTM probabilities
    if has_survival or has_probs:
        ax3 = axes[3]
        ax3.set_facecolor(BG)

        if has_survival:
            # ── Survival model: ŷ = S_7, y = 1 - true_onset_in_7d ───────────
            surv_w = wdf_w.dropna(subset=["S_7"]).sort_values("date")
            if not surv_w.empty:
                ax3.plot(surv_w["date"], surv_w["S_7"],
                         color="#2ecc71", linewidth=1.5, label="ŷ  S_7 (predicted)")
                if "true_onset_in_7d" in surv_w.columns:
                    true_surv = 1 - surv_w["true_onset_in_7d"].astype(float)
                    ax3.plot(surv_w["date"], true_surv,
                             color="#e74c3c", linewidth=1.5, alpha=0.8,
                             label="y  true 7d survival")
                if "S_30" in surv_w.columns:
                    ax3.plot(surv_w["date"], surv_w["S_30"],
                             color="#3498db", linewidth=1.0, alpha=0.6, linestyle="--",
                             label="S_30")
                ax3.axhline(0.5, color="white", linestyle="--", linewidth=0.8, alpha=0.4)
                ax3.legend(loc="lower left", facecolor=BG, labelcolor="white",
                           fontsize=8, framealpha=0)
            else:
                ax3.text(0.5, 0.5, "No survival predictions in this window",
                         ha="center", va="center", color="white",
                         transform=ax3.transAxes, fontsize=9)
            ax3.set_title("Survival Analysis  —  ŷ vs y", color="white", fontsize=10, pad=4)

        else:
            # ── Legacy LSTM probabilities ─────────────────────────────────────
...
def main():
    ...
    # ── Developer Activity + Survival Predictions chart ───────────────────────
    st.divider()
    st.subheader("Developer Activity")
    activity_df = _load_break_prediction_df(repo_full)
    if not activity_df.empty:
        _make_state_graph(activity_df, repo_full)
    else:
        st.info("No prediction data found. Run the pipeline first.")
