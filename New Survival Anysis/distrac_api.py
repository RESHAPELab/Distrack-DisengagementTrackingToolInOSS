...
def _load_parquet(org: str, repo: str, name: str) -> pd.DataFrame:
    path = _distrac(org, repo) / f"{name}.parquet"
    if not path.exists():
...
def _load_test_df(org: str, repo: str) -> pd.DataFrame:
    path = _repo_dir(org, repo) / "model_folder" / "test_df.csv"
    if not path.exists():
...
def _load_break_predictions(org: str, repo: str) -> pd.DataFrame:
    """Load survival model daily outputs from distrac/break_predictions.parquet."""
    path = _distrac(org, repo) / "break_predictions.parquet"
    if not path.exists():
        return pd.DataFrame()
...
def dev_timeline(org: str, repo: str, dev_id: str):
    ...
    # Load survival model daily predictions (break_predictions.parquet).
    # dev_id in break_predictions matches raw_dev (the test_df "dev" value).
    bp_df = _load_break_predictions(org, repo)
    bp_dict: dict = {}
    if not bp_df.empty and "dev_id" in bp_df.columns:
        bp_sub = bp_df[bp_df["dev_id"] == raw_dev].drop_duplicates("date", keep="last")
        bp_dict = bp_sub.set_index("date").to_dict("index")
    ...
    for _, r in sub.iterrows():
        ...
        # Overlay survival model predictions from break_predictions.parquet (if available).
        # These take priority over LSTM/classification model outputs for the bottom chart.
        bp_row = bp_dict.get(r["date"])
        if bp_row:
            for _scol in ("risk_30d", "S_7", "S_30", "inactivity_risk"):
                if pd.notna(bp_row.get(_scol)):
                    entry[_scol] = float(bp_row[_scol])
            # true_onset_in_7d is binary — keep as int
            if pd.notna(bp_row.get("true_onset_in_7d")):
                entry["true_onset_in_7d"] = int(bp_row["true_onset_in_7d"])
            # break_label from writer (renamed from true_onset_in_7d)
            if pd.notna(bp_row.get("break_label")) and "break_starts_in_14d" not in entry:
                entry["break_starts_in_14d"] = int(bp_row["break_label"])
...
def activity_window(org: str, repo: str,
    ...
    Sums commits/prs/issues across ALL developers for the selected range.
    """
    df = _load_test_df(org, repo)
    if start_date:
        df = df[df["date"].astype(str) >= start_date]
    if end_date:
        df = df[df["date"].astype(str) <= end_date]
    if df.empty:
        return {"n_weeks": 0, "total_commits": 0, "total_prs": 0, "total_issues": 0,
                "avg_commits_per_week": 0.0, "avg_prs_per_week": 0.0, "avg_issues_per_week": 0.0}
    # Count unique dates as n_weeks, then sum across all devs
    n_weeks = int(df["date"].nunique())
    totals = df[["commits", "prs", "issues"]].fillna(0).sum()
    tc, tp, ti = int(totals.get("commits", 0)), int(totals.get("prs", 0)), int(totals.get("issues", 0))
    return _clean({
        "n_weeks":              n_weeks,
        "total_commits":        tc,
        "total_prs":            tp,
...
def project_health(org: str, repo: str):
    path = _repo_dir(org, repo) / "ProjectHealth" / "project_health.json"
    if not path.exists():
...
def _kd_build_files_json(doe_df: pd.DataFrame) -> list:
    ...
    for file_path, group in doe_df.groupby("file_path"):
        ...
        for _, row in group.iterrows():
            short   = _kd_shorten_dev_id(str(row.get("dev_id", "")))
...
def departure_ph_panel(
    ...
    ph_path = _distrac(org, repo) / "project_health.json"
    if not ph_path.exists():
        ph_path = _repo_dir(org, repo) / "ProjectHealth" / "project_health.json"
    try:
...
def _compute_kd_for_date(org: str, repo: str, as_of_date_str: str):
    ...
    csv_path = _repo_dir(org, repo) / cfg.per_file_commits_path
    ...
    try:
        ...
        tf_devs_short = [_kd_shorten_dev_id(str(d)) for d in tf_list]

        result = (doe_df, tf_count, tf_devs_short)
...
def departure_kd_panel(org: str, repo: str, dev_id: str, as_of_date: Optional[str] = None):
    selected_short = _kd_shorten_dev_id(dev_id) if dev_id else None

    # On-demand recompute when a date is requested.
    if as_of_date:
        doe_df, tf_count, tf_devs_short = _compute_kd_for_date(org, repo, as_of_date)
    ...
    tf_count      = int(tf_df["tf"].iloc[0]) if len(tf_df) else 0
    tf_devs_short = (
        devs_df[devs_df["is_tf_dev"] == True]["login"]
        .dropna()
        .apply(lambda x: _kd_shorten_dev_id(str(x)))
        .tolist()
    )
    files_json = _kd_build_files_json(doe_df)
    if not files_json:
