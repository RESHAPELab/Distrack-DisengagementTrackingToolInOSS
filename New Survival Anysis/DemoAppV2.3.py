...
_SURV_LEAKY = _LEAKY_FEATURES | {"added_as_break", "state", "state_causal"}

def _emit_period(all_rows: list, buf: list, dev_id: str, period_num: int,
                 break_end: bool, feature_cols: list) -> None:
    """Append one counting-process row per buffered at-risk day to all_rows."""
    n = len(buf)
    for i, row in enumerate(buf):
        all_rows.append({
            "dev_id":      f"{dev_id}_p{period_num}",
            "date":        row["date"],
            "start_day":   i,
            "stop_day":    i + 1,
            "event":       1 if (i == n - 1 and break_end) else 0,
            "tenure_days": i,
            **{col: row.get(col, 0.0) for col in feature_cols},
        })


def build_survival_dataset(df: pandas.DataFrame, feature_cols: list) -> pandas.DataFrame:
    """Convert per-dev-day timeline to per-period counting-process format.

    Each contiguous at-risk run (ACTIVE/NON_CODING) becomes a separate subject
    ``{dev}_p{n}`` with its own time clock reset to 0 at period start.

    - Every day inside a period: event=0
    - Last day of the period:    event=1 if period ends in INACTIVE/GONE, else 0

    This avoids the 1-day-ahead sparsity of the single-clock formulation (where
    only the single day immediately before a break is positive) and gives the
    baseline hazard a concrete shape: "days since last return to activity."
    Each period contributes exactly one event or one censoring, which is the
    correct structure for CoxTimeVaryingFitter.

    Note: the ``dev_id`` field in the output is ``{original_dev}_p{n}``.
    Callers that need the original developer key for merging should strip the
    ``_p\\d+$`` suffix (see write section in DemoAppV2.3.py).
    """
    df = df.copy()
    df["date"] = pandas.to_datetime(df["date"])
    df = df.sort_values(["dev", "date"]).reset_index(drop=True)

    at_risk_states  = {"ACTIVE", "NON_CODING"}
    inactive_states = {"INACTIVE", "GONE"}

    all_rows: list = []
    for dev_id, dev_df in df.groupby("dev"):
        dev_df     = dev_df.sort_values("date").reset_index(drop=True)
        next_states = dev_df["state"].shift(-1).fillna("CENSORED")

        period_num = 0
        period_buf: list = []  # pandas Series rows buffered for current period

        for idx in range(len(dev_df)):
            state      = dev_df.iloc[idx]["state"]
            next_state = next_states.iloc[idx]

            if state in at_risk_states:
                period_buf.append(dev_df.iloc[idx])
                if next_state in inactive_states:          # period ends in break
                    _emit_period(all_rows, period_buf, dev_id, period_num,
                                 break_end=True, feature_cols=feature_cols)
                    period_buf = []
                    period_num += 1
            elif period_buf:                               # safety: flush open period
                _emit_period(all_rows, period_buf, dev_id, period_num,
                             break_end=False, feature_cols=feature_cols)
                period_buf = []
                period_num += 1

        if period_buf:                                     # flush trailing period
            _emit_period(all_rows, period_buf, dev_id, period_num,
                         break_end=False, feature_cols=feature_cols)

    result = pandas.DataFrame(all_rows)
    for col in feature_cols + ["tenure_days"]:
        if col in result.columns:
            result[col] = pandas.to_numeric(result[col], errors="coerce").fillna(0.0)
    return result


def bucket_risk(r: float) -> str:
...
def generate_survival_predictions(ctv, surv_df, feature_cols, horizons=(7, 14, 30, 60, 90)):
    """
    Conditional survival probabilities at fixed horizons for each developer's
    most recent observation:

        P(still active in h days | active today, X) =
            exp( -[H0(t+h) - H0(t)] * exp(beta*X) )

    Returns DataFrame: dev_id, prediction_day, S_7, S_14, S_30, S_60, S_90, risk_30d, risk_band
    """
    bch      = ctv.baseline_cumulative_hazard_.squeeze()
    last_obs = surv_df.sort_values("stop_day").groupby("dev_id").tail(1).copy()

    preds = []
    for _, row in last_obs.iterrows():
        current_t = int(row["stop_day"])
        X = row[feature_cols].to_frame().T.astype(float)
        try:
            log_ph = float(ctv.predict_log_partial_hazard(X).iloc[0])
        except Exception:
            continue
        ph    = np.exp(log_ph)
        h_now = float(np.interp(current_t, bch.index.values, bch.values,
                                left=0.0, right=float(bch.values[-1])))
        entry = {"dev_id": row["dev_id"], "prediction_day": current_t}
        for h in horizons:
            h_t = float(np.interp(current_t + h, bch.index.values, bch.values,
                                  left=0.0, right=float(bch.values[-1])))
            entry[f"S_{h}"] = float(np.exp(-ph * max(h_t - h_now, 0.0)))
        entry["risk_30d"]  = 1.0 - entry["S_30"]
        entry["risk_band"] = bucket_risk(entry["risk_30d"])
        preds.append(entry)
    return pandas.DataFrame(preds)


def plot_survival_curves_st(ctv, surv_df, feature_cols, n_sample=6):
    """
    Predicted 0-180 day conditional survival curves for a sample of developers.
    Dotted verticals = actual break onset days in the data.
    """
    bch   = ctv.baseline_cumulative_hazard_.squeeze()
    devs  = surv_df["dev_id"].unique()[:n_sample]
    tgrid = np.arange(0, 181)

    ncols = 2
    nrows = max(1, (len(devs) + 1) // ncols)
    fig, axes = plt.subplots(nrows, ncols, figsize=(13, 3.5 * nrows), constrained_layout=True)
    axes = np.array(axes).ravel()

    for ax, dev_id in zip(axes, devs):
        dev_rows  = surv_df[surv_df["dev_id"] == dev_id].sort_values("start_day")
        last_row  = dev_rows.iloc[-1]
        current_t = int(last_row["stop_day"])
        X         = last_row[feature_cols].to_frame().T.astype(float)
        try:
            log_ph = float(ctv.predict_log_partial_hazard(X).iloc[0])
        except Exception:
        ...
        h_now = float(np.interp(current_t, bch.index.values, bch.values,
                                left=0.0, right=float(bch.values[-1])))
        curve = [np.exp(-ph * max(
                    float(np.interp(current_t + t, bch.index.values, bch.values,
                                    left=0.0, right=float(bch.values[-1]))) - h_now, 0.0))
                 for t in tgrid]

        ax.plot(tgrid, curve, lw=2, label="S(t|X)")
        ax.axvline(30, color="orange", lw=1, ls="--", alpha=0.7, label="30d")
        ax.axvline(90, color="red",    lw=1, ls="--", alpha=0.7, label="90d")
        for ed in dev_rows[dev_rows["event"] == 1]["start_day"].values:
            ahead = ed - current_t
            if 0 <= ahead <= 180:
                ax.axvline(ahead, color="black", lw=1.5, ls=":", alpha=0.8)
        ax.set_title(str(dev_id)[:30], fontsize=8)
        ax.set_xlabel("Days from last observation", fontsize=8)
        ax.set_ylabel("P(still active)", fontsize=8)
        ax.set_ylim(0, 1.05)
        ax.legend(fontsize=7)

    for ax in axes[len(devs):]:
        ax.set_visible(False)
    st.pyplot(fig)
    plt.close(fig)


def evaluate_survival_model_st(ctv, test_surv, feature_cols):
    """
    Harrell's C-index on the test set + baseline survival / hazard plots.
    C-index: 0.5 = random, >0.7 = good.
    """
    from lifelines.utils import concordance_index
    X = test_surv[feature_cols].astype(float)
    try:
        log_ph = ctv.predict_log_partial_hazard(X).values.ravel()
    except Exception as e:
        st.warning(f"Partial hazard prediction failed: {e}")
        return

    try:
        c_idx = concordance_index(test_surv["stop_day"].values, -log_ph,
                                  test_surv["event"].values)
        c1, c2 = st.columns(2)
        c1.metric("Harrell's C-index (test)", f"{c_idx:.3f}",
                  help="0.5 = random; 1.0 = perfect; >0.70 is good")
        if c_idx >= 0.70:
            c2.success("Good discrimination (>= 0.70)")
        elif c_idx >= 0.60:
            c2.info("Moderate discrimination (0.60-0.70)")
        else:
            c2.warning("Weak discrimination (< 0.60)")
    except Exception as e:
        st.warning(f"C-index failed: {e}")

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    ctv.baseline_survival_.plot(ax=axes[0])
    axes[0].set_title("Baseline Survival S0(t)")
    axes[0].set_xlabel("Tenure Day")
    axes[0].set_ylabel("S0(t)")
    ctv.baseline_cumulative_hazard_.plot(ax=axes[1])
    axes[1].set_title("Baseline Cumulative Hazard H0(t)")
    axes[1].set_xlabel("Tenure Day")
    axes[1].set_ylabel("H0(t)")
    plt.tight_layout()
    st.pyplot(fig)
    plt.close(fig)


def generate_daily_survival_predictions(ctv, surv_df, feature_cols, horizons=(7, 30)):
    """
    Score EVERY row in surv_df (each dev-day at-risk) — not just the last observation.

    For each row at tenure time t with features X:
        S_h = exp( -exp(beta*X) * [H0(t+h) - H0(t)] )

    Also computes true_onset_in_7d: 1 if a real break onset happens within the next
    7 calendar days (time-aware window, not row count).  Used for back-testing.

    Returns surv_df with added columns: S_7, S_30, risk_30d, risk_band, true_onset_in_7d
    """
    bch    = ctv.baseline_cumulative_hazard_.squeeze()
    X      = surv_df[feature_cols].astype(float)
    log_ph = ctv.predict_log_partial_hazard(X).values.ravel()
    ph_arr = np.exp(log_ph)
    times  = surv_df["start_day"].values

    h_now = np.interp(times, bch.index.values, bch.values,
                      left=0.0, right=float(bch.values[-1]))

    result = surv_df.copy()
    for h in horizons:
        h_fut = np.interp(times + h, bch.index.values, bch.values,
                          left=0.0, right=float(bch.values[-1]))
        result[f"S_{h}"] = np.exp(-ph_arr * np.maximum(h_fut - h_now, 0.0))

    result["risk_30d"] = 1.0 - result["S_30"]
    result["risk_band"] = result["risk_30d"].apply(bucket_risk)

    # True 7-day onset: for each row at day t, did a real event happen in (t, t+7]?
    result = result.sort_values(["dev_id", "start_day"]).reset_index(drop=True)
    onset_col = pandas.Series(0, index=result.index)
    for dev_id, grp in result.groupby("dev_id"):
        event_days = grp.loc[grp["event"] == 1, "start_day"].values
        starts     = grp["start_day"].values
        if len(event_days) > 0:
            in_win = (starts[:, None] < event_days[None, :]) & \
                     (event_days[None, :] <= starts[:, None] + 7)
            onset_col.loc[grp.index] = in_win.any(axis=1).astype(int)
    result["true_onset_in_7d"] = onset_col.values
    return result


def plot_daily_survival_ts(daily_df, n_devs=4):
    """
    Back-test time series: predicted S_7 (blue) vs true 7-day survival (orange)
    for a sample of developers.  Red dashes = actual break onset days.

    If predicted S_7 drops before/at actual break onsets, the model is working.
    """
    devs = daily_df["dev_id"].unique()[:n_devs]
    fig, axes = plt.subplots(len(devs), 1, figsize=(14, 3.5 * len(devs)),
                             constrained_layout=True)
    if len(devs) == 1:
        axes = [axes]

    for ax, dev_id in zip(axes, devs):
        grp = daily_df[daily_df["dev_id"] == dev_id].sort_values("start_day")
        if grp.empty or "S_7" not in grp.columns:
            ax.set_visible(False)
            continue

        use_dates = "date" in grp.columns
        x = pandas.to_datetime(grp["date"].values) if use_dates else grp["start_day"].values

        ax.plot(x, grp["S_7"].values, lw=1.5, color="steelblue",  label="Predicted S_7")
        if "true_onset_in_7d" in grp.columns:
            ax.plot(x, 1 - grp["true_onset_in_7d"].values, lw=1.5,
                    color="darkorange", alpha=0.75, label="True 7d survival")

        event_x = x[grp["event"].values == 1]
        for ex in event_x:
            ax.axvline(ex, color="red", lw=0.9, ls="--", alpha=0.6)
        ...
        ax.set_title(f"{str(dev_id)[:50]}", fontsize=9)
...
def view_df(df, name="DataFrame", max_rows=10_000):
    ...
    _safe_prefix = re.sub(r'[^A-Za-z0-9_-]', '_', name)[:40]
    csv_file = tempfile.NamedTemporaryFile(
        "w", delete=False, suffix=".csv", encoding="utf-8",
        prefix=f"{_safe_prefix}_"
    )
...
def diagnose_survival_frame(df: pd.DataFrame,
                            feature_cols: list,
                            event_col: str = "event") -> pd.DataFrame:
    """
    Find what's making CoxTimeVaryingFitter blow up.
    Returns a per-column report sorted by severity.
    """
    rows = []
    for col in feature_cols:
        ...
        if not np.issubdtype(s.dtype, np.number):
            rows.append({"column": col, "dtype": str(s.dtype),
                         "n_nan": int(s.isna().sum()), "n_pos_inf": 0, "n_neg_inf": 0,
                         "min": None, "max": None, "std": None,
                         "n_unique": int(s.nunique(dropna=False)), "notes": "NON-NUMERIC"})
            continue

        n_nan     = int(s.isna().sum())
        n_pos_inf = int(np.isposinf(s).sum())
        n_neg_inf = int(np.isneginf(s).sum())
        finite    = s[np.isfinite(s)]

        col_min = float(finite.min())  if len(finite) else None
        col_max = float(finite.max())  if len(finite) else None
        col_std = float(finite.std())  if len(finite) else None
        n_uniq  = int(s.nunique(dropna=False))

        notes = []
        if n_nan:                                       notes.append(f"NaN×{n_nan}")
        if n_pos_inf:                                   notes.append(f"+inf×{n_pos_inf}")
        if n_neg_inf:                                   notes.append(f"-inf×{n_neg_inf}")
        if col_std is not None and col_std == 0:        notes.append("ZERO VARIANCE")
        elif col_std is not None and col_std < 1e-8:    notes.append("near-zero var")
        if n_uniq == 1:                                 notes.append("CONSTANT")
        if col_max is not None and abs(col_max) > 1e8:  notes.append(f"huge max={col_max:.2e}")
        if col_min is not None and abs(col_min) > 1e8:  notes.append(f"huge min={col_min:.2e}")

        rows.append({"column": col, "dtype": str(s.dtype),
                     "n_nan": n_nan, "n_pos_inf": n_pos_inf, "n_neg_inf": n_neg_inf,
                     "min": col_min, "max": col_max, "std": col_std,
                     "n_unique": n_uniq, "notes": "; ".join(notes) if notes else "ok"})

    report = pd.DataFrame(rows).sort_values(
        ["n_nan", "n_pos_inf", "n_neg_inf", "n_unique"],
        ascending=[False, False, False, True]
    ).reset_index(drop=True)

    # ── Summary ────────────────────────────────────────────────────────────
    total_nan = report["n_nan"].sum()
    total_inf = report["n_pos_inf"].sum() + report["n_neg_inf"].sum()
    n_zero_var = (report["notes"].str.contains("ZERO VARIANCE", na=False)).sum()
    n_constant = (report["notes"].str.contains("CONSTANT", na=False)).sum()
    n_huge     = (report["notes"].str.contains("huge", na=False)).sum()
    print(f"Rows in frame:        {len(df):>10,}")
    print(f"Total NaN:            {total_nan:>10,}")
    print(f"Total inf (±):        {total_inf:>10,}")
    print(f"Zero-variance cols:   {n_zero_var:>10}")
    print(f"Constant cols:        {n_constant:>10}")
    print(f"Huge-magnitude cols:  {n_huge:>10}")
    return report


def find_collinear_pairs(df: pd.DataFrame, feature_cols: list,
                         threshold: float = 0.99) -> pd.DataFrame:
    """Find near-perfectly correlated feature pairs (Cox can't fit these)."""
    numeric = df[feature_cols].select_dtypes(include=[np.number])
    # drop zero-variance cols first or corr gives NaN
    ...
    for i, a in enumerate(cols):
        for b in cols[i+1:]:
            if corr.loc[a, b] >= threshold:
                pairs.append({"col_a": a, "col_b": b, "corr": float(corr.loc[a, b])})
    return pd.DataFrame(pairs).sort_values("corr", ascending=False).reset_index(drop=True)
...
def main():
    ...
    if st.button("Predictors"):
        ...
        for repo in st.session_state.list_of_repos:
            ...
            # 6d — Encode expanded break-algorithm predictors as numeric
            # These columns come from label_timeline() + diagnostics merge and are already
            # in the labeled timeline; we just ensure they are numeric before saving.
            if "added_as_break" in enriched.columns:
                enriched["added_as_break"] = (enriched["added_as_break"] == "yes").astype(float)
            if "break_day" in enriched.columns:
                enriched["break_day"] = pandas.to_numeric(enriched["break_day"], errors="coerce").fillna(0).astype(float)
            for col in ["th", "len", "win_th"]:
                if col in enriched.columns:
                    enriched[col] = pandas.to_numeric(enriched[col], errors="coerce").fillna(0)
    ...
    if st.button("Run Survival Prediction"):
        ...
        train_raw = pandas.concat(train_frames, ignore_index=True)
        test_raw  = pandas.concat(test_frames,  ignore_index=True) if test_frames else pandas.DataFrame()
        st.info(f"Loaded {train_raw['dev'].nunique()} train devs from {len(train_frames)} repos"
                + (f" | {test_raw['dev'].nunique()} test devs from {len(test_frames)} repos"
                   if not test_raw.empty else " | no test repos"))
        ...
        _base_cols = [
            ...
            'pr_activity', 'coding_day', 'nc_day',
            # ── break-algorithm features ──────────────────────────────────────
            'break_day',            # 1 if this day falls inside a detected break
            'len',                  # length of the surrounding break (days)
            ...
            'files_worked_today', 'owned_files_today', 'collab_files_today',
            'collab_commit_ratio',
        ...
        feature_cols = [c for c in _base_cols if c in train_raw.columns]
        st.info(f"Features ({len(feature_cols)}): {feature_cols}")

        # ── C. Build counting-process survival dataset ────────────────────────
        with st.spinner("Building survival dataset (counting-process format)..."):
            train_surv = build_survival_dataset(train_raw, feature_cols)
            test_surv  = build_survival_dataset(test_raw, feature_cols) if not test_raw.empty else pandas.DataFrame()

        st.info(
            f"Train: {len(train_surv):,} rows | {train_surv['event'].sum():,} events "
            f"({train_surv['event'].mean()*100:.1f}% event rate)"
        )

        # ── D. Fit CoxTimeVaryingFitter ───────────────────────────────────────
        from lifelines import CoxTimeVaryingFitter
        with st.spinner("Fitting CoxTimeVaryingFitter..."):
            ctv = CoxTimeVaryingFitter(penalizer=0.1)
            fit_cols = ["dev_id", "start_day", "stop_day", "event"] + feature_cols
            
            report = diagnose_survival_frame(train_surv[fit_cols], feature_cols=feature_cols)

            collin = find_collinear_pairs(train_surv, feature_cols, threshold=0.99)

            # ── Auto-clean: drop zero-variance and collinear columns ──────────
            _stds = train_surv[feature_cols].std()
            _drop_zero = _stds[_stds == 0].index.tolist()
            if _drop_zero:
                st.warning(f"Dropping {len(_drop_zero)} zero-variance column(s): {_drop_zero}")
                feature_cols = [c for c in feature_cols if c not in _drop_zero]

            _drop_collinear = {row["col_b"] for _, row in collin.iterrows()} if not collin.empty else set()
            if _drop_collinear:
                st.warning(f"Dropping {len(_drop_collinear)} perfectly collinear column(s): {list(_drop_collinear)}")
                feature_cols = [c for c in feature_cols if c not in _drop_collinear]

            fit_cols = ["dev_id", "start_day", "stop_day", "event"] + feature_cols

            # Scale features: Cox doesn't auto-scale and mixed-scale inputs
            # (binary 0/1 vs counts in the thousands) badly condition the Hessian.
            # Fit on train only; transform test with the same scaler.
            scaler = StandardScaler()
            train_surv = train_surv.copy()
            train_surv[feature_cols] = scaler.fit_transform(train_surv[feature_cols])
            if not test_surv.empty:
                test_surv = test_surv.copy()
                test_surv[feature_cols] = scaler.transform(test_surv[feature_cols])
            st.session_state["surv_scaler"] = scaler

            ctv.fit(
                train_surv[fit_cols],
                id_col="dev_id", event_col="event",
                start_col="start_day", stop_col="stop_day",
                show_progress=False,
            )
        st.success("CoxTimeVaryingFitter fitted.")


        # ── E. Generate predictions ───────────────────────────────────────────
        if not test_surv.empty:
            # ── E1. Daily predictions — every dev-day scored ─────────────────
            with st.spinner("Scoring every dev-day in test set..."):
                daily_df = generate_daily_survival_predictions(
                    ctv, test_surv, feature_cols, horizons=(7, 30)
                )

            display_cols = ["dev_id", "date", "start_day", "event",
                            "true_onset_in_7d", "S_7", "S_30", "risk_30d", "risk_band"]
            display_cols = [c for c in display_cols if c in daily_df.columns]

            # ── E2. Back-test: predicted S_7 vs true 7-day survival ──────────
            st.subheader("Back-test — Predicted S_7 vs True 7-day Survival")
            st.caption(
                "Blue = model's predicted probability of staying active for 7 more days. "
                "Orange = actual ground truth (1 if no break in next 7 days, 0 if break). "
                "Red dashes = actual break onset days. "
                "Good model: blue drops toward 0 just before red dashes."
            )
            plot_daily_survival_ts(daily_df, n_devs=4)

            # ── E3. Current-risk summary — last observation per dev ───────────
            pred_df = generate_survival_predictions(ctv, test_surv, feature_cols)
            st.subheader("Current Risk Summary — one row per developer")
            view_df(pred_df, "surv_pred_df")

            # ── E4. Survival curves + evaluation ─────────────────────────────
            st.subheader("Survival Curves — 6 sample developers")
            st.caption(
                "Each curve = P(developer still active N days from their last observation). "
                "Dotted verticals = actual break onsets in the test data."
            )
            plot_survival_curves_st(ctv, test_surv, feature_cols, n_sample=6)

            st.subheader("Evaluation")
            evaluate_survival_model_st(ctv, test_surv, feature_cols)

            # ── E5. Write to distrac/ per test repo ──────────────────────────
            # Merge _repo and state back so the writer can split per repo.
            # build_survival_dataset uses "{dev}_p{n}" for period IDs; strip the
            # suffix so the merge key matches the original "dev" column.
            _repo_state_map = test_raw[["dev", "date", "_repo", "state"]].copy()
            _repo_state_map["date"] = pandas.to_datetime(_repo_state_map["date"])
            _daily_write = daily_df.copy()
            _daily_write["date"] = pandas.to_datetime(_daily_write["date"])
            _daily_write["dev_id"] = _daily_write["dev_id"].str.replace(
                r"_p\d+$", "", regex=True
            )
            _daily_write = _daily_write.merge(
                _repo_state_map.rename(columns={"dev": "dev_id"}),
                on=["dev_id", "date"], how="left",
            )
            for _org, _repo in test_pairs:
                _repo_key  = f"{_org}/{_repo}"
                _repo_slice = _daily_write[_daily_write["_repo"] == _repo_key].copy()
                if _repo_slice.empty:
                    st.warning(f"No daily predictions for {_repo_key} — skipping write.")
                    continue
                dw.write_distrac_stage2(
                    repo_full_name=_repo_key,
                    predictions_df=_repo_slice,
                )
                st.success(f"Saved survival predictions for {_repo_key} ({_repo_slice['dev_id'].nunique()} devs)")

            view_df(daily_df)
            st.session_state["surv_ctv"]       = ctv
            st.session_state["surv_pred_df"]   = pred_df
            st.session_state["surv_daily_df"]  = daily_df
            st.session_state["surv_feat_cols"] = feature_cols
            st.session_state["surv_test_surv"] = test_surv
        else:
            st.warning("No test repos — skipping prediction/evaluation.")

    # ── Survival predictions explorer — survives reruns ──────────────────────
    if "surv_pred_df" in st.session_state:
        _spd = st.session_state["surv_pred_df"]
        st.subheader("Survival Explorer (persists across reruns)")
        _sc1, _sc2, _sc3 = st.columns(3)
        _sc1.metric("Developers", str(_spd["dev_id"].nunique()))
        if "risk_band" in _spd.columns:
            _rc = _spd["risk_band"].value_counts()
            _sc2.metric("CRITICAL / HIGH risk", str(_rc.get("CRITICAL", 0) + _rc.get("HIGH", 0)))
            _sc3.metric("LOW / MEDIUM risk",    str(_rc.get("LOW", 0) + _rc.get("MEDIUM", 0)))
