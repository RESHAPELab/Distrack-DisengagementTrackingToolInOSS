...
def calculate_metrics(edge_df: pandas.DataFrame) -> pandas.DataFrame:
    """
    Step 4: Compute per-user summary metrics from the edge list.

    These metrics describe how connected each developer is:

        degree          — number of unique collaborators
        weighted_degree — sum of all edge weights (total shared threads)
        issue_degree    — weighted degree counting only issue threads
        pr_degree       — weighted degree counting only PR threads
        top_collaborator        — the single most-connected partner by total weight
        top_collaborator_weight — edge weight to that top partner

    Returns a DataFrame with one row per developer, sorted by weighted_degree
    descending (most central developers first).
    """
    if edge_df.empty:
    ...
    user_edges: dict[str, list] = defaultdict(list)
    for _, row in edge_df.iterrows():
        a, b = row["developer_a"], row["developer_b"]
        user_edges[a].append((b, row["weight_issues"], row["weight_prs"], row["weight_total"]))
        user_edges[b].append((a, row["weight_issues"], row["weight_prs"], row["weight_total"]))

    rows = []
    for user, edges in user_edges.items():
        degree          = len(edges)
        weighted_degree = sum(e[3] for e in edges)
        issue_degree    = sum(e[1] for e in edges)
        pr_degree       = sum(e[2] for e in edges)
        top = max(edges, key=lambda e: e[3])
        rows.append({
            "user":                    user,
            "degree":                  degree,
            "weighted_degree":         weighted_degree,
            "issue_degree":            issue_degree,
            "pr_degree":               pr_degree,
            "top_collaborator":        top[0],
            "top_collaborator_weight": top[3],
        })

    metrics_df = pandas.DataFrame(rows)
    ...
    # Betweenness centrality (normalized: 0 = no paths through this node, 1 = all paths)
    bc = nx.betweenness_centrality(G, normalized=True, weight="weight")
    metrics_df["betweenness_centrality"] = metrics_df["user"].map(bc).fillna(0.0).round(4)

    # Articulation point: True if removing this node increases number of components
    art_points = set(nx.articulation_points(G))
    metrics_df["is_articulation_point"] = metrics_df["user"].isin(art_points)

    # ── Role classification (4-class, p75 thresholds) ───────────────────────
    deg_p75 = metrics_df["degree"].quantile(0.75)
    bc_p75  = metrics_df["betweenness_centrality"].quantile(0.75)

    def _classify_role(row):
        hi_deg = row["degree"] >= deg_p75
        hi_bc  = row["betweenness_centrality"] >= bc_p75
        if hi_deg and hi_bc:
            return "Maintainer"
        elif hi_deg:
            return "Collaborator"
    ...
    metrics_df["role"] = metrics_df.apply(_classify_role, axis=1)
    return metrics_df
