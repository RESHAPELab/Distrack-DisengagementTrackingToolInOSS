...
def mock_collect_repo(repo):
    total_prs = np.random.randint(10, 20)
    total_issues = np.random.randint(10, 20)


    pb_commit = tqdm(total=40, desc="Commits", position=0, leave=True)
    ...
    for _ in range(20):
        random = np.random.uniform(0.4, 0.6)
        time.sleep(random)
        pb_commit.update(1)
        time.sleep(np.random.uniform(0.01, 0.4))
        pb_pr.update(1)
        time.sleep(np.random.uniform(0.01, 0.4))
        pb_issue.update(1)
        time.sleep(np.random.uniform(0.01, 0.4))
    ...
    for _ in range(total_prs):
        random = np.random.uniform(0.2, 0.3)
        time.sleep(random)
        pb_pr.update(1)

    for _ in range(total_issues):
        random = np.random.uniform(0.1, 0.4)
        time.sleep(random)
        pb_issue.update(1)

    #using the repo read a csv file

    path = ORG_BASE / repo / "commit_list.csv"
    st.write("Reading commit list from:", path)
    df = pandas.read_csv(path)

    st.success("Data collection complete (demo).")
...
def label_developers_activity(repo) -> pandas.DataFrame:
    """
...
def users_activity_all_repos(
    ...
    print(f"Loading repos timelines for {repos}")
...
def add_targets(segments: pandas.DataFrame) -> pandas.DataFrame:
    """
    Adds two target columns to segments:
...
def predict_state(repo , tf_devs, use_all=True):
    ...
    with st.expander("Per-segment predictions (held-out)"):
        # Keep the selected repo + TF devs for the demo

        sel = out[
            (out["repo"].astype(str).eq(repo)) &
            (out["author"].astype(str).isin(set(tf_devs)))
        ].copy()

        # nice columns to surface
        show_cols = [
            "author","repo","segment_id","start_date","end_date", "state_curr","next_state", "pred_next_state", "current_break_length","pred_current_break_len","ae_current_break_len","next_break_length","pred_next_break_len","ae_next_break_len"
        ]
        show_cols = [c for c in show_cols if c in sel.columns]
        sel = sel.sort_values(["author","start_date","segment_id"]).reset_index(drop=True).dropna()
        #Remove the 00:00:00+00:00
        sel["start_date"] = sel["start_date"].dt.strftime('%Y-%m-%d')
        sel["end_date"] = sel["end_date"].dt.strftime('%Y-%m-%d')
        st.dataframe(sel[show_cols].tail(200))
        st.caption(f"Saved full table to: {out_csv}")
...
if st.button("Collect Data Demo"):
        data = mock_collect_repo(repo_key)
...
if st.button("Run labeling"):
    tf_devs = label_developers_activity(repo_key)
    ...
    st.write(f"User timeline for {tf_devs[0]}", user_timeline)
